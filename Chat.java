...
public class Chat {
	public static void main(String[] args) throws Exception{
		...
		try {
			...
			while (true){
				...
				//string method:split(regular expression, limit times)
	...
	public static String getMyIp(){
		String ip = null;
		try {
			//return all the interfaces on this machine(or an enumeration of NetworkInterfaces found on this machine).
			...
			for (NetworkInterface iface : Collections.list(interfaces)) {
				...
				// isUp()return whether network interface is up and running
				if (iface.isLoopback() || !iface.isUp()) continue;

				Enumeration<InetAddress> addresses = iface.getInetAddresses();
				for (InetAddress addr : Collections.list(addresses)){
					//filters out Ipv6
					//check if addr is one type of ipv6
	...
	public static class PeerConnection extends Thread{
		...
		public void run(){
			try{
				...
				//DataOutputStream return the numbers of bytes written to data output stream
		...
		public void sendMessage(String message) {
			try {
				//DataOutputStream method writeInt(int v) writeUTF(String)
				out.writeInt(2);
		...
		public String getHost(){
			//getInetAddress() return ip address of remote, getHostAddress() return string format
